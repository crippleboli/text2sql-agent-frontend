        ...
        <div class="bubble">
          ...
          <div v-else-if="msg.type === 'steps'" class="steps">
            <div v-for="(step, sIdx) in msg.steps" :key="sIdx" class="step">
              ...
              <span :class="step.status">{{ step.text }}</span>
            </div>
          </div>
...
async function sendQuestion() {
  ...
  const stepIndex =
    messages.value.push({
      role: "assistant",
      type: "steps",
      // 不预置步骤：首个阶段事件到达前只显示通用的“思考中”指示
      steps: [{ text: "正在思考…", status: "running", waiting: true }],
    }) - 1;
    ...
    while (true) {
      ...
      for (const evt of events) {
        ...
        if (data.stage) {
          ...
          const wi = steps.findIndex((s) => s.waiting);
          if (wi !== -1) {
            // 首个真实步骤到达：替换“正在思考”占位
            steps[wi] = { text: data.stage, status: "running" };
          } else {
            // 后续步骤：执行到了才追加
            steps.push({ text: data.stage, status: "running" });
          }
        } else if (data.error) {
          // 正在执行的步骤标记异常（含首个事件到达前的等待占位）
          steps.forEach((s) => {
            if (s.status === "running") {
              s.status = "error";
              if (s.waiting) s.text = "执行失败";
            }
          });
          messages.value.push({
            role: "assistant",
